...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force)
{
	...
	if (reload_fonts())
	{
		...
		fonts[LOGS] = create_font(crypt_str("Smallest Pixel-7"), 12, FW_DONTCARE, FONTFLAG_ANTIALIAS); //Raleway
		fonts[ESP] = create_font(crypt_str("Small Fonts"), 8, FW_MEDIUM, FONTFLAG_OUTLINE); //Smallest Pixel-7
