...
std::string get_config_dir()
{
	...
	if (SUCCEEDED(SHGetFolderPath(NULL, 0x001a, NULL, NULL, path)))
		folder = crypt_str("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\luas");
...
void lua_edit(std::string window_name)
{
	...
	auto get_dir = [&]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
			file_path = std::string(path) + crypt_str("\\hentaiware\\scripts\\");
			...
			if (active_tab == 1)
			{
				if (rage_tab == 0)
				{
					tab_start("1", column_1, 0, size_a_x, size_d_y);
					{
						...
						ImGui::SameLine();
						draw_keybind(crypt_str(""), &g_cfg.ragebot.enable_key, crypt_str("##HOTKEY_ENABLED_RAGEBOT"));
					...
					tab_start("2", column_2, 0, size_a_x, size_a_y);
					{
						...
						if (g_cfg.ragebot.double_tap)
						{
							...
							ImGui::Checkbox(crypt_str("Instant DT"), &g_cfg.ragebot.instant_double_tap);
						}
					}
					tab_end();
				}
				else if (rage_tab == 1)
				{
					...
					tab_start("3221", column_1, 0, size_a_x, size_d_y);
					{
						...
						if (g_cfg.ragebot.weapon[hooks::rage_weapon].adaptive_point_scale == false)
						{
					...
					tab_start("weapon#1", column_2, 0, size_a_x, size_a_y);
					{
						...
						{
							ImGui::Checkbox(crypt_str("Adaptive shot damage"), &g_cfg.ragebot.weapon[hooks::rage_weapon].adaptive_shot_dmg);
						}
						...
						if (g_cfg.ragebot.autowall)
							ImGui::SliderInt(crypt_str("Wall minimum damage"), &g_cfg.ragebot.weapon[hooks::rage_weapon].minimum_damage, 1, 120, true);
					...
					tab_start("accuracy", column_2, size_a_y + 5, size_a_x, size_a_y);
					{
						...
						if (g_cfg.ragebot.double_tap && hooks::rage_weapon <= 4)
						{
							...
							//draw_multicombo(crypt_str("Double-tap conditions"), g_cfg.ragebot.weapon[hooks::rage_weapon].double_tap_modifiers, autostop_modifiers, ARRAYSIZE(doubletap_modifiers), preview);
						}
					}
					tab_end();
				}
				...
				tab_start("lua#1", column_1, 0, size_a_x, size_d_y);
				{
					...
					if (ImGui::CustomButton(crypt_str("Open scripts folder"), crypt_str("##LUAS__FOLDER"), ImVec2(220 * dpi_s, 26)))
					{
						...
						auto get_dir = [&folder]() -> void
						{
							...
							if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
								folder = std::string(path) + crypt_str("\\hentaiware\\scripts\\");
