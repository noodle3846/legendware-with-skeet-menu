...
void aim::scan(adjust_data* record, scan_data& data, const Vector& shoot_position, bool optimized)
{
	...
	auto minimum_damage = get_minimum_damage(true, record->player->m_iHealth()); //	auto minimum_damage = get_minimum_damage(false, record->player->m_iHealth());
...
std::vector <scan_point> aim::get_points(adjust_data* record, int hitbox, bool from_aim)
{
	...
	else
	{
		...
		if (c_config::get()->m["rage_mphitbox"][0] || c_config::get()->m["rage_mphitbox"][1] || c_config::get()->m["rage_mphitbox"][2] || c_config::get()->m["rage_mphitbox"][3] || c_config::get()->m["rage_mphitbox"][4])
		{
			if (hitbox == HITBOX_HEAD)
