...
void otheresp::indicators()
{
	...
	Color PINGCOL = percent_col((float(std::clamp(g_ctx.globals.framerate, 30, 200) - 64)) / 200.f * 100.f);
...
void otheresp::create_fonts() {
	...
	IndFont = create_font2("Calibri", 30 * CMenu::get()->GetDPINum(), 610, FONTFLAG_ANTIALIAS);
	IndShadow = create_font2("Calibri", 30 * CMenu::get()->GetDPINum(), 610, FONTFLAG_ANTIALIAS);
}
...
void otheresp::automatic_peek_indicator()
{
	...
	if (!weapon->is_non_aim() && c_config::get()->b["rage_quick_peek_assist"] && c_config::get()->auto_check(c_config::get()->i["rage_quickpeek_enabled"], c_config::get()->i["rage_quickpeek_enabled_style"]) || alpha)
	{
		...
		auto color_main = Color(c_config::get()->c["rage_quick_peek_assist_col"][0], c_config::get()->c["rage_quick_peek_assist_col"][1], c_config::get()->c["rage_quick_peek_assist_col"][2], (int)((alpha * 197) / 3));
