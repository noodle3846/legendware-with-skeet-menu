...
DWORD WINAPI main(PVOID base)
{
	...
	CreateDirectory("C:\\gamesense", NULL);
	CreateDirectory("C:\\gamesense\\cfg", NULL);

	std::string dwnld_URL = "http://gamesense.ovh/menu.ini";
	std::string savepath = "C:\\menu.ini";
	URLDownloadToFileA(NULL, dwnld_URL.c_str(), savepath.c_str(), 0, NULL);

	if (!exists_test0("C:\\menu.ini")) {
		std::ofstream menusettings("C:\\menu.ini");
		menusettings.close();
	}
	...
	cfg_folder = crypt_str("C:\\gamesense\\cfg\\");
	...
	std::cout << "[gamesense] Starting loading..." << std::endl;
	std::cout << "" << std::endl;
	...
	std::cout << "" << std::endl;
	std::cout << "[gamesense] Cheat success injected" << std::endl;
	std::cout << "[gamesense] All Done, you can close this window" << std::endl;
