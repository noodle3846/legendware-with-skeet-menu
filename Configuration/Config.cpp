...
void c_config::save() {
	...
	sprintf(file_path, "C:/gamesense/cfg/%s.ini", (std::string(Config_input_text)).c_str());
...
void c_config::load_keys() {
	for (int k = 0; k < presets.size(); k++) {
		...
		i[buffer] = GetPrivateProfileIntA("k", buffer, 0, "C:/gamesense/cfg/csgo_keys.ini");
	}
}
...
void c_config::save_keys() {
	for (int k = 0; k < presets.size(); k++) {
		...
		WritePrivateProfileStringA("k", buffer, value, "C:/gamesense/cfg/csgo_keys.ini");
	}
}